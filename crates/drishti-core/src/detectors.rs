...
impl AnomalyDetector for MemoryLeakDetector {
    ...
    fn evaluate(&self, current: &JvmSnapshot, history: &[JvmSnapshot]) -> Vec<Alert> {
        ...
        // R²
        let mean_y = sy / n;
        let ss_tot: f64 = recent
            .iter()
            .map(|s| (s.heap.used as f64 - mean_y).powi(2))
            .sum();
        let ss_res: f64 = recent
            .iter()
            .enumerate()
            .map(|(i, s)| {
                let p = slope * i as f64 + intercept;
                (s.heap.used as f64 - p).powi(2)
            })
            .sum();
...
impl AnomalyDetector for GcPressureDetector {
    ...
    fn evaluate(&self, current: &JvmSnapshot, history: &[JvmSnapshot]) -> Vec<Alert> {
        ...
        if history.len() >= 2 {
            ...
            let total_gc_ms_now: i64 = current
                .gc_collectors
                .iter()
                .map(|c| c.collection_time_ms)
                .sum();
            let total_gc_ms_first: i64 = first
                .gc_collectors
                .iter()
                .map(|c| c.collection_time_ms)
                .sum();
            ...
            if uptime_delta > 0.0 {
                ...
                if throughput < 0.95 {
                    let severity = if throughput < 0.85 {
                        Severity::Critical
                    } else if throughput < 0.90 {
                        Severity::High
                    } else {
                        Severity::Warn
                    };
        ...
        let full_gc_count: i64 = current
            .gc_collectors
            .iter()
            .filter(|c| {
                c.name.to_lowercase().contains("old") || c.name.to_lowercase().contains("full")
            })
            .map(|c| c.collection_count)
            .sum();
        if full_gc_count > 0 && current.jvm_info.uptime_ms > 60000 {
            let full_gc_per_min =
                full_gc_count as f64 / (current.jvm_info.uptime_ms as f64 / 60000.0);
            if full_gc_per_min > 1.0 {
                alerts.push(Alert {
                    ...
                    detail: format!(
                        "{:.1} Full GCs per minute ({} total)",
                        full_gc_per_min, full_gc_count
                    ),
                    evidence_tab: EvidenceTab::Memory,
                    first_seen: Utc::now(),
                    suppressed_until: None,
    ...
    fn evaluate(&self, current: &JvmSnapshot, history: &[JvmSnapshot]) -> Vec<Alert> {
        ...
        if let Some(ref h) = current.hikari {
            ...
            if h.timeout_count > 0 {
                let prev_timeouts = history
                    .last()
                    .and_then(|s| s.hikari.as_ref())
                    .map(|h| h.timeout_count)
                    .unwrap_or(0);
