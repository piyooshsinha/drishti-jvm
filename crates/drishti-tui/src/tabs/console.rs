...
impl ConsoleTab {
    ...
    fn cmd_help(&mut self) {
        let commands = vec![
            ...
            (
                "threads [id]",
                "List threads or show stack trace for thread ID",
            ),
    ...
    fn cmd_dashboard(&mut self) {
        ...
        self.push_output(
            ...
            Style::default().fg(if heap_pct > 85.0 {
                Color::Red
            } else {
                Color::White
            }),
        ...
        self.push_output(
            &format!("  GC Throughput: {:.1}%", derived.gc_throughput * 100.0),
            Style::default(),
        );
        self.push_output(
            &format!(
                "  HTTP: {:.1} req/s, {:.1}ms avg",
                derived.http_requests_per_sec, snap.http.avg_latency_ms
            ),
            Style::default(),
        );
    ...
    fn cmd_threads(&mut self, id_str: Option<&str>) {
        ...
        if let Some(id_s) = id_str {
            if let Ok(id) = id_s.parse::<i64>() {
                if let Some(t) = snap.threads.iter().find(|t| t.id == id) {
                    self.push_output(
                        &format!("Thread #{}: {} [{:?}]", t.id, t.name, t.state),
                        Style::default().fg(Color::Cyan),
                    );
                    self.push_output(
                        &format!(
                            "  daemon={} blocked={} waited={}",
                            t.daemon, t.blocked_count, t.waited_count
                        ),
                        Style::default(),
                    );
                    if let Some(ref lock) = t.lock_name {
                        self.push_output(
                            &format!("  waiting on: {}", lock),
        ...
        self.push_output(
            &format!("=== Threads ({}) ===", snap.threads.len()),
            Style::default().fg(Color::Cyan).bold(),
        );
        ...
        if snap.threads.len() > 30 {
            self.push_output(
                &format!("  ... and {} more", snap.threads.len() - 30),
                Style::default().fg(Color::DarkGray),
            );
        }
    }
    ...
    fn cmd_gc(&mut self) {
        ...
        for c in &snap.gc_collectors {
            ...
            self.push_output(
                &format!(
                    "  {}: {} collections, {}ms total, {:.1}ms avg",
                    c.name, c.collection_count, c.collection_time_ms, avg
                ),
                Style::default(),
            );
        }
        ...
        if !gc_events.is_empty() {
            self.push_output(
                &format!("\n=== Recent GC Events ({}) ===", gc_events.len()),
                Style::default().fg(Color::Cyan).bold(),
            );
            for ev in gc_events.iter().rev().take(10) {
                let color = if ev.phase == drishti_core::model::GcPhase::FullGc {
                    Color::Red
                } else {
                    Color::White
                };
                self.push_output(
                    &format!(
                        ...
                        ev.cause,
                        ev.pause_ms,
                        ev.heap_before_bytes / 1_048_576,
                        ev.heap_after_bytes / 1_048_576
                    ),
                    Style::default().fg(color),
                );
            }
    ...
    fn cmd_heap(&mut self) {
        ...
        self.push_output(
            &format!("  Used:      {:.1} MB", snap.heap.used_mb()),
            Style::default(),
        );
        self.push_output(
            &format!(
                "  Committed: {:.1} MB",
                snap.heap.committed as f64 / 1_048_576.0
            ),
            Style::default(),
        );
        self.push_output(
            &format!("  Max:       {:.1} MB", snap.heap.max_mb()),
            Style::default(),
        );
        self.push_output(
            &format!("  Usage:     {:.1}%", pct),
    ...
    fn cmd_uptime(&mut self) {
        ...
        self.push_output(
            &format!(
                "  VM:      {} {}",
                snap.jvm_info.vm_name, snap.jvm_info.vm_version
            ),
            Style::default(),
        );
        self.push_output(
            &format!("  Vendor:  {}", snap.jvm_info.vm_vendor),
            Style::default(),
        );
        self.push_output(
            &format!("  Java:    {}", snap.jvm_info.spec_version),
            Style::default(),
        );
        self.push_output(
            &format!("  Uptime:  {}", snap.jvm_info.uptime_human()),
            Style::default(),
        );
        self.push_output(
            &format!("  GC:      {}", snap.jvm_info.gc_algorithm_str()),
            Style::default(),
        );
    }
    ...
    fn cmd_alerts(&mut self) {
        ...
        if alerts.is_empty() {
            ...
            for a in &alerts {
                ...
                self.push_output(
                    &format!("  [{}] {} — {}", a.severity, a.title, a.detail),
                    Style::default().fg(color),
                );
            }
        }
    ...
    pub fn draw(&self, frame: &mut Frame, area: Rect) {
        ...
        frame.render_widget(Paragraph::new(input_display).block(input_block), chunks[1]);
