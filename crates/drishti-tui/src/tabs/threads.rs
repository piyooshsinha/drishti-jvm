...
impl ThreadsTab {
    pub fn draw(&self, frame: &mut Frame, area: Rect) {
        ...
        if has_deadlock {
            ...
            let banner =
                Paragraph::new(format!(" ⚠ DEADLOCK DETECTED: {} threads involved", total))
        ...
        let offset = self
            .scroll_offset
            .min(total_threads.saturating_sub(visible_height));
        ...
        let scroll_info = if total_threads > 0 {
            format!(
                " Threads {}-{} of {} (j/k to scroll) ",
                offset + 1,
                (offset + visible_height).min(total_threads),
                total_threads
            )
        } else {
