...
pub struct OverviewTab { pub state: Arc<AppState> }
...
impl OverviewTab {
    pub fn draw(&self, frame: &mut Frame, area: Rect) {
        ...
        frame.render_widget(Gauge::default()
            ...
            .label(format!("{:.0}M / {:.0}M ({:.0}%)", snap.heap.used_mb(), snap.heap.max_mb(), heap_pct)),
            r1[0]);

        let cpu_pct = snap.cpu.process_cpu_pct();
        frame.render_widget(Gauge::default()
            .block(Block::default().title(" Process CPU ").borders(Borders::ALL)
            ...
            .label(format!("Proc:{:.0}% Sys:{:.0}% Cores:{}", cpu_pct, snap.cpu.system_cpu_pct(), snap.cpu.available_processors)),
            r1[1]);

        let deadlock_str = if snap.deadlocks.is_empty() { "" } else { " ⚠DEADLOCK" };
        let thread_block = Block::default().title(" Threads ").borders(Borders::ALL);
        let dl_style = if snap.deadlocks.is_empty() { Style::default() } else { Style::default().fg(Color::Red) };
        let ts = &snap.thread_summary;
        let thread_text = vec![
            Line::from(format!("  Live:{} Daemon:{} Peak:{}{}", ts.live, ts.daemon, ts.peak, deadlock_str)),
            Line::from(format!("  RUN:{} WAIT:{} BLOCK:{} TW:{}",
                ts.state_counts.get(&drishti_core::model::ThreadState::Runnable).unwrap_or(&0),
                ts.state_counts.get(&drishti_core::model::ThreadState::Waiting).unwrap_or(&0),
                ts.state_counts.get(&drishti_core::model::ThreadState::Blocked).unwrap_or(&0),
                ts.state_counts.get(&drishti_core::model::ThreadState::TimedWaiting).unwrap_or(&0))),
        ];
        frame.render_widget(Paragraph::new(thread_text).block(thread_block).style(dl_style), r1[2]);
        ...
        let tp_color = if throughput_pct < 95.0 { Color::Red } else if throughput_pct < 98.0 { Color::Yellow } else { Color::Green };
        let mut gc_lines = vec![
            Line::from(format!("  Throughput: {:.1}%  Overhead: {:.2}%", throughput_pct, derived.gc_overhead * 100.0))
        ...
        for c in snap.gc_collectors.iter().take(2) {
            ...
            gc_lines.push(Line::from(format!("  {}: {} ({:.1}ms avg)", c.name, c.collection_count, avg)));
        }
        ...
        let http_text = vec![
            Line::from(format!("  {:.1} req/s  Errors: {}", derived.http_requests_per_sec, snap.http.total_errors))
                .style(if snap.http.total_errors > 0 { Style::default().fg(Color::Red) } else { Style::default() }),
            Line::from(format!("  Avg latency: {:.1}ms  Endpoints: {}", snap.http.avg_latency_ms, snap.http.endpoints.len())),
        ];
        frame.render_widget(Paragraph::new(http_text).block(http_block), r2[1]);

        // HikariCP
        let pool_block = Block::default().title(" Connection Pool ").borders(Borders::ALL);
        let pool_text = if let Some(ref h) = snap.hikari {
            let util = h.utilization_pct();
            vec![
                Line::from(format!("  Active:{}/{} Idle:{} Pending:{}", h.active, h.max, h.idle, h.pending)),
                Line::from(format!("  Util:{:.0}% Timeouts:{}", util, h.timeout_count))
                    .style(if h.is_saturated() { Style::default().fg(Color::Red) } else { Style::default() }),
            ]
        } else { vec![Line::from("  No HikariCP data")] };
        ...
        let alert_lines: Vec<Line> = if alerts.is_empty() {
            ...
            alerts.iter().take(max_alerts).map(|a| {
                ...
                Line::from(format!("  [{}] {} — {}", a.severity, a.title, a.detail)).style(Style::default().fg(color))
            }).collect()
        };
