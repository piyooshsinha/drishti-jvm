    ...
    pub fn draw(&self, frame: &mut Frame, area: Rect) {
        ...
        frame.render_widget(
            Gauge::default()
                ...
                .percent(nh_pct.clamp(0.0, 100.0) as u16)
                .label(format!(
                    "{:.0}M / {:.0}M",
                    snap.non_heap.used_mb(),
                    snap.non_heap.max_mb()
                )),
            top[1],
        );
        ...
        let pool_rows: Vec<Row> = snap
            ...
            .map(|p| {
                let pct = p
                    ...
                    .map(|v| format!("{:.0}%", v))
                    .unwrap_or("-".into());
                let type_str = match p.pool_type {
                    PoolType::Heap => "Heap",
                    PoolType::NonHeap => "Non-Heap",
                    _ => "?",
                };
                let color = p
                    .usage
                ...
                Row::new(vec![
                    p.name.clone(),
                    type_str.to_string(),
                    format!("{:.1}M", p.usage.used_mb()),
                    format!("{:.1}M", p.usage.max_mb()),
                    pct,
                ])
        ...
        let gc_rows: Vec<Row> = snap
            ...
            .map(|c| {
                ...
                Row::new(vec![
                    c.name.clone(),
                    c.collection_count.to_string(),
                    format!("{:.0}ms", c.collection_time_ms),
                    format!("{:.1}ms", avg),
                ])
            })
            .collect();
